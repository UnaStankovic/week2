...
describe('Custom Tests', function () {
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const aliceEthAddress = '0x1ABC7154748D1CE5144478CDEB574AE244B939B5'
    const aliceWithdrawAmount = utils.parseEther('0.07')
    const aliceChangeUtxo2 = new Utxo({
      amount: aliceDepositAmount.sub(aliceWithdrawAmount).sub(bobWithdrawAmount),
      keypair: aliceKeypair,
    })
    await transaction({
      tornadoPool,
    ...
    const tornadoPoolBalance = await token.balanceOf(tornadoPool.address)
    expect(tornadoPoolBalance).to.be.equal(aliceDepositAmount.sub(aliceWithdrawAmount).sub(bobWithdrawAmount))
  })
})
